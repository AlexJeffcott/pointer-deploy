...
2. The server checks the fixed paths first: `/healthz`, `/compositions`, `/units`, `/assets*`. None match (`src/server/index.ts:79-99`).
...
8. From here the handler is a branch, not a line (`src/server/index.ts:138-175`). Four gates decide the rest of this section. A plain first visit passes A and B and stops at C:

    | Gate | Test | A failure means |
    | --- | --- | --- |
    | A | `manifest.schema === 3` | no switcher and no override; straight to step 12 |
...
10. Past gate B. `optionsFor` marks every id the channel has served and every published id as current / live / disabled, and computes each one's `since` from the entry below it. Past gate D, `compose` rebuilds the manifest around `chosen`.
...
12. One document, `cache-control: no-store, must-revalidate`. It contains, in order (`src/server/html.ts:230-243`):
...
27. `Shell.tsx` reads `__APPS__` and `__VERSIONS__` at module scope, once.
...
30. First paint: masthead, name and colour inputs, tabs, the version row, and two empty slots marked `data-app-loading`. `Versions()` renders nothing where `__VERSIONS__` is empty, which is the gate A or B page (`src/web/shell/Shell.tsx:54-56`).
