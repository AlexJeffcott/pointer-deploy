...
**The CORS allow-list covers what a page does today and not what step 7 adds.** The service answers `access-control-allow-methods: GET, POST, OPTIONS` and `access-control-allow-headers: content-type`, which is what a push and a pull need - a `POST` carrying JSON is preflighted, and both are on the list. Moving a slot is `PUT` with `x-write-key`, and neither is allowed yet, so step 7 adds both or the browser refuses the request before it is sent. Measured from the service's own constant rather than from a browser, because no page in this tree makes that request.
