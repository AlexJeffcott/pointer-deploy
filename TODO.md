...
**A mutation this branch declined to write, and why it is not in the array.** Cutting the Monday offset out of `weekOf` makes the week start today - which on a Monday is a no-op, so the mutation would be caught on six days in seven and the reading would depend on what day the suite was run. What is in the array shifts every day by one whatever the date is. The scenario that catches it asserts three things about the seven days: consecutive, day 1 a Monday, and today among them.
...
So roughly **420 ms** of the 822 is neither fetch, where the first reading put it at 530 ms of 832. The fetches got slower and the baseline did not, which narrows the gap without explaining it. It is not module evaluation or rendering: those happen in both arms, and the warm arm's whole reading is 45 to 68 ms.

**And a second unexplained reading arrived with step 5.** `week`'s warm arm ran 46, 60, 51, 46, 92, 45, 106, 113, 121 ms across nine runs - it CLIMBS, where `board`'s stays inside 46 to 68 on the same machine in the same session. The control arm is flat at 822 to 850 and the two fetches do not climb either. The median is 60 ms and the benefit is 779 ms, so nothing about the warm turns on it. What the climb is, nothing has measured. It may be the same thing as the 420 ms and it may not, and this item holds both because neither has a reading.
...
| What is not known | Where the other 420 ms goes, and why `week`'s warm arm climbs. Candidates nothing has measured: the delay between a stylesheet's resource timing ending and its `onload` firing, which is what `addStylesheet` awaits; SRI verification on a cross-origin file; the route change and the effect that starts the load |
| Why it matters to the design | `loader.ts:44-45` awaits the stylesheet and THEN imports the module. A `Promise.all` there would cut the control arm towards the LONGER of the two rather than their sum - about 200 ms of 822, not half of it - and it needs no warm tag anywhere. The 767 to 779 ms is not at risk from it; what is at risk is the sentence that says a serial loader is why the baseline is what it is |
| The next reading | Time `addStylesheet`'s promise and the `import()` separately in the page, in both arms and for both units. That is the measurement that says whether the 420 ms is the loader's or the browser's, and whether the climb is one of them |
