 ...
 *
 * Every unit but the named one keeps the baseline marker, so its id is the id
 * the baseline build produced and `publish` has nothing to upload for it. That
 * is what makes "only this unit was uploaded" a reading rather than a hope.
 */
...
// Every unit but the sub-app the scenario moved. Named by exclusion rather than
// as a literal, so a third unit at `PLAN.md` step 4 is covered without an edit.
Then("the {word} channel still serves build {string} for the shell", async function (this: PointerWorld, channel: string, name: string) {
  ...
  for (const unit of UNITS.filter((u) => u === "shell")) {
...
/**
 * Publishes a `list` unit by hand, with its record patched.
 *
 * No build can produce one: every unit `build.ts` emits carries the contract set
 * and the member set it was measured with. The bytes are a bundle that does
 * nothing, and what is under test is the promote's refusal, not the bundle.
 */
async function publishIncompatible(
  ...
  const prefix = `units/list/${id}`;
  const baseline = world.unitIdOf("one", "list");

  const source = await fetch(
    `https://${cfg.bucket}.${new URL(cfg.endpoint).host}/units/list/${baseline}/unit.json`,
  );
  const manifest = (await source.json()) as Record<string, unknown> & { files: string[] };

  await putObject(cfg, `${prefix}/${manifest.js as string}`, new TextEncoder().encode("export default function(){return null}\n"), {
...
When("the operator promotes that unit to the {word} channel", async function (this: PointerWorld, channel: string) {
  this.lastRun = await this.promoteUnit(channel as Channel, "list", fresh.get("incompatible")!);
});
...
Then(
  ...
  function (this: PointerWorld) {
    ...
    expect(this.lastRun?.stderr).toContain(
      "list uses ShellStore.teleport, which this shell does not have",
    );
    expect(this.lastRun?.stderr).toContain("Nothing was changed");
