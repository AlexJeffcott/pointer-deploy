Feature: Deploying and rolling back one unit at a time
  ...
  Scenario: Deploying a sub-app leaves the frame where it was
    Given a new "hello" unit is published
    When the operator promotes that "hello" unit to the qa channel
    Then visitors to the qa origin receive the new "hello" unit within the propagation window
  ...
  Scenario: Deploying the frame leaves the sub-app at its new version
    Given a new "hello" unit is published
    And that "hello" unit is already deployed to the qa channel
    ...
    And the qa channel still serves the new "hello" unit
  ...
  Scenario: Rolling one unit back leaves the other at its newer version
    ...
    When the operator promotes build "one"'s "hello" unit to the qa channel
    Then visitors to the qa origin receive build "one"'s "hello" unit within the propagation window
  ...
  Scenario: Publishing after a change to one unit uploads that unit alone
    When the operator builds and publishes with only "hello" changed
    Then only the hello unit is uploaded
  ...
  Scenario: Promoting an unpublished unit is refused
    When the operator promotes a "hello" unit that was never published
