...
**It is generated, and it is not committed.** `CHANGELOG.md` is in `.gitignore`. Every fact in it is already in `deploys/`, which IS committed, for the reason the archive exists at all: the pointer is overwritten by the next promote, its history is 20 deep, and the store was rewritten whole once already. A generated copy of facts already in git earns nothing and costs three things - it goes stale between the record and the regeneration, it needs a check to catch that, and two branches that each land a deploy conflict on its counts. The first version defended that check at length and never defended the commit.

**So what has to be right is the archive and the readings.** `scripts/record.test.ts` holds every decision an entry makes, and `scripts/changelog.test.ts` holds `readArchive` against fixtures it builds in a temporary directory: which directories count as records, which files count as pointer bytes, what an unreadable `deploys/` means, and that a record from another archive is labelled by that archive. That last one is why there had been no fixture test at all - `dir` was hardcoded to `deploys/<name>` whatever archive was read, so a fixture rendered links to files that do not exist. The loader was the untested half, and it was the half that had been wrong twice.

**The note that says nothing is the one worth refusing.** `shoot` writes `notes.md` and, when nobody passed `--note`, writes `TODO: one line saying what this deploy demonstrates.` into it. So the reachable failure is a placeholder and not an empty file, and a check that refused only the empty one would pass over every unwritten note there has ever been. `noteRefusal` refuses both, and refuses neither on a record `shoot` never wrote: `promote` writes no `notes.md`, and `shoot` cannot reach `prod` at all. Demanding one everywhere would have made the first `prod` deploy a permanently red suite over a record this design says is correct.

**A promote that moved nothing is not a deploy anybody asked for, and the entry says so.** Four of the five records in the archive are promotes made to exercise the recorder itself - a pointer rewritten to the composition it already named - and each one's note says so. Presenting them as deploys would be a false reading of the archive by the one document whose whole claim is that it restates it. So the reading is taken from `promote.json` rather than from the prose beside it: a promote every one of whose units is `carried` moved nothing, its entry is titled `nothing moved`, and it carries a sentence saying that a no-op promote is a real write to a real channel and not a deploy anybody asked for. The summary counts the three kinds, and where no record it can read has moved a unit it says so in a line of its own - **over the records it can read, and no further.** The first version wrote "No record in this archive moved a unit" over an archive holding one record with no act at all. That is a universal claim the generator cannot support, in the one document whose whole argument is that a reading comes off `promote.json` and not off prose, and it was false in the committed file. The same shape applied to warnings, where a `promote.json` carrying no `warnings` field was counted as a promote that printed nothing. §34 carries the underlying problem, which is that the archive is mostly its own verification traffic; this makes that visible rather than fixing it.

**A record with no act still gets an entry.** The oldest one predates `promote.json`: pointer bytes and pictures, and nothing saying what was run. Its rows read `not recorded` rather than `nothing`, which are two different claims. The difference matters most on `warnings`: `[]` is a promote that printed nothing, a missing field is a record that could not have printed anything, and the archive holds four of the first and one of the second. A `promote.json` that names no unit is a third state again - a schema this tree has not seen yet - and it gets its own sentence, because telling a reader the file is absent above a table quoting its command is the document contradicting itself.

**A record can hold pointer bytes and no act.** `writeRecord` writes the manifests before `promote.json` and awaits neither, so a promote that moved a real pointer can leave its bytes behind with nothing beside them. That directory is a record and gets an entry, with its regions read from the files on disk. Skipping it - which the first version of this did, under a comment about empty directories git cannot hold anyway - would have dropped from the document exactly the record the archive exists for.

**A one-region promote does not read as a deploy of every region.** `deploys/2026-09-10T16-51-30Z-qa` was written by `--region eu`. `manifest.eu.json` holds the bytes it PUT; `manifest.us.as-served.json` is what `us` was serving, which is an earlier promote's pointer under a name that says so. `promote.json` names one region, so the entry names one region and then names the region this promote did not write. **That reading comes from the region list, not from the files the record kept.** `promoteRecord` builds `manifests` from the regions it wrote, so for a record with no `shots.json` the two are identical by construction and the reading vanished - on `prod`, which can never be shot, and where the promote record is the whole record. It survived here only because this particular record happened to be shot. Reading the kept manifests as the regions the act wrote would claim a deploy that did not happen, and the manifests are what a reader has in front of them.

**A reading is taken off the ids, not off the word beside them.** `promote.json` labels each unit `carried`, `moved`, `new` or `dropped`, and it also records `from` and `unitId`. The label is prose with a schema; the ids are what it was derived from. `stateOf` recomputes from the ids, so a record whose label disagrees with its own ids is read on the ids - and the entry says which unit disagreed rather than resolving it in silence.

**`new` means the composition held no id for that unit, and not that the unit was never promoted.** The changelog says **added** for exactly that reason. It said "first promote" until 2026-09-11, and the archive already held a record where that was false: `deploys/2026-09-10T21-15-37Z-qa` puts `hello 3bba892b` back eight minutes after a promote removed it, so `from` is null and the state is `new` - for a unit that had been served for weeks. A record sees one pointer and its predecessor; it cannot see a unit's history and must not word itself as though it could.

**What it does not claim.** It restates the archive and checks nothing against the world. An entry saying `qa` served a composition is a statement about `deploys/` and not about what the pointer holds now - `bun run pr` is what compares the newest record to the live pointer, and it refuses rather than re-shooting. Nothing checks that the archive holds every promote that happened: a promote nobody committed is in neither, and the `git cat-file` reading in `bun run pr` is what catches that, one pull request later. It opens none of the images it links, so a link to a picture that was deleted renders exactly like one to a picture that is there. Nothing here re-reads a manifest, opens a browser or contacts the store.

Measured on 2026-09-10: five records render to 6.5 kB, and the four-of-five and no-unit-moved readings above are that day's archive rather than standing facts - the generated file counts them itself, which is the point.

**Read cold twice on 2026-09-10 by `devils-advocate-agent`.** The first reading found thirteen defects in the first version. The `prod` note requirement, the placeholder note, a false sentence in the generated header, the silently skipped record, an unattributable JSON crash, a `readdir` failure that would have overwritten the document and exited 0, `Composed at` showing the moment of a screenshot, "predates `promote.json`" above a table quoting a `promote.json`, a movement read off the label, a duplicate heading for two records in one second, a preview that reached the archive reading as a deploy, a summary range taken from the sort key rather than from the instants, and a shot entry missing a field throwing inside the generator: all fixed here, each with the test that would have caught it. `scripts/falsify.ts` carries five mutations over these readings, which is a sample and not a score - and §34 now carries the score, because it was measured: 81.12% over `scripts/record.ts`, with 198 mutants surviving.

The second reading found ten more. The sharpest was the generated file itself: a false universal claim on line 11, in the artefact, of exactly the kind this exercise exists to prevent. That, the untested loader, the region reading that depended on having been shot, the summary flattening `none` and `not recorded`, `cell()` applied to eight of thirteen call sites, and a placeholder note described as no note at all: fixed here. It also asked why the file was committed at all, which is the question that dissolved four of its own findings once the answer turned out to be that it should not be.
