...
**It is generated, and that is a check rather than a sentence at the top of the file.** A generated file whose generator nobody runs stops describing its source in silence, and the discovery comes months later in the shape of an entry that is not there. `scripts/changelog.test.ts` renders `deploys/` again and fails when what is on disk differs, so a record committed without running `bun run changelog` is a red test. It runs under the ordinary `bun test` and there is no CI here, so what actually holds it is `CLAUDE.md`'s rule that `bun test` runs before a pull request - one rule covering every check in the tree, rather than one more convention about this file.
...
**A one-region promote does not read as a deploy of every region.** `deploys/2026-09-10T16-51-30Z-qa` was written by `--region eu`. `manifest.eu.json` holds the bytes it PUT; `manifest.us.as-served.json` is what `us` was serving, which is an earlier promote's pointer under a name that says so. `promote.json` names one region, so the entry names one region and then names the region this promote did not write. Reading the kept manifests as the regions the act wrote would claim a deploy that did not happen, and the manifests are what a reader has in front of them.
...
**Read cold on 2026-09-10 by `devils-advocate-agent`,** which found thirteen defects in the first version. The `prod` note requirement, the placeholder note, a false sentence in the generated header, the silently skipped record, an unattributable JSON crash, a `readdir` failure that would have overwritten the document and exited 0, `Composed at` showing the moment of a screenshot, "predates `promote.json`" above a table quoting a `promote.json`, a movement read off the label, a duplicate heading for two records in one second, a preview that reached the archive reading as a deploy, a summary range taken from the sort key rather than from the instants, and a shot entry missing a field throwing inside the generator: all fixed here, each with the test that would have caught it. `scripts/falsify.ts` carries five mutations over these readings, which is a sample and not a score.
